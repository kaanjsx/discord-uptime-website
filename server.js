...
const app = express();
const helmet = require("helmet"); // discord.gg/developers
...
app.locals.domain = "domain callbacksiz";
...
client.on("ready", () => {
  client.user.setActivity(`by kaan#1337`);
  passport.use(
    new Strategy(
      {
...
linkss = linkA
 ...
 if(db.get("linkler").map(z => z.url).includes(link)) {
      ...
      res.send("Link Başarıyla Eklendi");
    }
  });
